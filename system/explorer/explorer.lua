...
function show_explorer_list()
	...
	--screen.print(5+movx,5,Root[Dev],1,theme.style.TITLECOLOR,color.black,__ALEFT)--+2
	...
	if scroll.list.maxim > 0 then
		...
		screen.print(10+movx,80,"...".."\n"..strings.back,1.1,color.white,theme.style.DATETIMECOLOR)
	end
...
function ctrls_explorer_list()
	...
	if buttons[cancel] then -- return directory
		if (Root[Dev]=="ux0:" or Root[Dev]=="ux0:/") or (Root[Dev]=="ur0:" or Root[Dev]=="ur0:/") then return end
...
function show_msg_vpk(infovpk)
	...
	while true do
		...
		if sfoPVK then
			screen.print(960/2,y+12,tostring(sfoPVK.TITLE),1,color.black,color.blue,__ACENTER)
		...
		if os.message(strings.required+files.sizeformat(sizeocuped)+strings.searchapp,1)==1 then
			...
			if moveapp then
				...
				if os.message(strings.apptour0+"\n"+applist.data[posapp].name+"\n"+strings.tittle+tittleapp+" ?",1)==1 then
					reboot=false
					local mov = game.move(applist.data[posapp].name, 1)
					if mov == 1 then appmove = true
					else
...
local installgame_callback = function ()
	if #explorer.list > 0 then
		...
		while true do
			...
			if info then
				screen.print(960/2,y+25,tostring(info.TITLE),1,color.black,color.blue,__ACENTER)
				screen.print(960/2,y+50,tostring(info.TITLE_ID),1,color.black,color.blue,__ACENTER)
			end
			if img then
...
local installtheme_callback = function ()
	if #explorer.list > 0 then
		...
		buttons.homepopup(0)
		reboot=false
		local result = themes.install(explorer.list[scroll.list.sel].path)
		buttons.homepopup(1)
		reboot=true
...
local sizedir_callback = function ()
	if #explorer.list > 0 then
		if explorer.list[scroll.list.sel].multi then
			...
			if not explorer.list[scroll.list.sel].size then 					-- Its Dir
				local sizedir= files.size(explorer.list[scroll.list.sel].path)
...
function menu_ctx.wakefunct(var)
	...
	menu_ctx.scroll = newScroll(menu_ctx.options, #menu_ctx.options)--10)
end
